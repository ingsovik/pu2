 ...
 */
    function isActiveInMonth(chosenMonth: number | null, startMonth: number, endMonth: number) {
 ...
 */
    function isActiveOnDay(chosenDay: number | null, chosenMonth: number | null, startDay: number, endDay: number, startMonth: number) {
        let booleanActive = false;
    ...
    /**
     * Function for logging number of active issues for the chosen month of date variabel. 
     * Values saved to activeIssues array
     */
    function findActiveIssuesForMonth() {
        activeIssues.pop();
        const chosenMonthAsNumber = date.getMonth();
        for (let i = 1; i <= getDaysInMonth(date); i++) {
            const numberOfActiveIssues = calcNumActiveIssuePerDay(i, chosenMonthAsNumber)
            if (numberOfActiveIssues != null) {
                if (numberOfActiveIssues > 0) {
                    activeIssues.push({ x: i, y: numberOfActiveIssues });
                } else {
    ...
    return (
        <div>
            <h2 className="graphH">Active issues in selected month</h2>
            <p>Choose any day  in the wanted month from the calendar</p>
            {findActiveIssuesForMonth()}
